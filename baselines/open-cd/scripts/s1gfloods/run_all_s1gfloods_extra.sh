...
CALLER_PWD="$(pwd)"
TTP_CONFIG="configs/ttp/ttp_vit-sam-b_256x256_40k_s1gfloods.py"
TTP_SAM_BASE_CKPT="${OPENCD_DIR}/pretrained/vit-base-p16_sam-pre_3rdparty_sa1b-1024px_20230411-2320f9cc.pth"

CONFIGS=(
  "configs/changeformer/changeformer_mit-b0_256x256_40k_s1gfloods.py"
  "configs/cgnet/cgnet_256x256_40k_s1gfloods.py"
  "configs/stanet/stanet_pam_256x256_40k_s1gfloods.py"
  "configs/snunet/snunet_c16_256x256_40k_s1gfloods.py"
  "configs/hanet/hanet_256x256_40k_s1gfloods.py"
  "${TTP_CONFIG}"
)

usage() {
  cat <<'EOF'
Usage: run_all_s1gfloods_extra.sh <check-env|smoke-train|full-train> [options]
...
check_env() {
  ...
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/mpl_opencd_s1gfloods_extra}"
  ...
  if printf '%s\n' "${CONFIGS[@]}" | grep -Fxq "${TTP_CONFIG}"; then
    if [[ ! -f "${TTP_SAM_BASE_CKPT}" ]]; then
      echo "Missing TTP base checkpoint: ${TTP_SAM_BASE_CKPT}" >&2
      echo "Please place the ViT-SAM-B checkpoint under ${OPENCD_DIR}/pretrained/." >&2
      exit 1
    fi
    echo "TTP base checkpoint: ${TTP_SAM_BASE_CKPT}"
  fi
...
mods = ['mmcv', 'mmseg', 'mmengine', 'mmpretrain', 'opencd', 'torch', 'einops']
vers = {}
for m in mods:
...
try:
    import peft
    vers['peft'] = getattr(peft, '__version__', 'unknown')
    print(f'peft=={vers["peft"]}')
except Exception as e:
    raise SystemExit(
        'Incompatible peft/transformers for TTP. '
...
build_train_cmd() {
  ...
  if [[ "${config}" == "${TTP_CONFIG}" ]]; then
    # TTP 统一使用仓库内的本地 ViT-SAM-B 权重，避免 batch 训练时访问远程 checkpoint。
    extra_cfg_opts+=(
      "model.backbone.encoder_cfg.init_cfg.checkpoint=${TTP_SAM_BASE_CKPT}"
    )
  fi
...
run_batch_train() {
  ...
  for config in "${CONFIGS[@]}"; do
    ...
    if [[ "${status_code}" -eq 0 ]]; then
      ...
      printf '%s\tsuccess\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "${model_tag}" "${status_code}" "${config}" "${work_dir}" "${best_ckpt}" \
        "${log_file}" "${started_at}" "${ended_at}" "${duration_sec}" >> "${summary_file}"
  ...
  echo
  echo "Batch finished. Success: ${success_count}, Failed: ${failed_count}"
