...
class _PAMBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.ds != 1:
            x = self.pool(input)
        batch_size, _, h, w2 = x.size(0), x.size(1), x.size(2), x.size(3)
        w = w2 // 2
...
class CDSA(nn.Module):
    ...
    def __init__(self, in_c, ds=1, mode='BAM'):
        ...
        if self.mode == 'BAM':
            ...
            self.Self_Att = PAM(
                in_channels=self.in_C, out_channels=self.in_C, sizes=[1, 2, 4, 8], ds=self.ds)
        elif self.mode == 'None':
...
class STAHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        dist = F.interpolate(
            dist, size=inputs[0].shape[2:], mode='bilinear', align_corners=True)
