...
class CMCD(BaseModule):
    def __init__(self,
        ...
        self.opt_in_channels = int(opt_in_channels)
        self.sar_in_channels = int(sar_in_channels)
    ...
    def forward(self, x):
        ...
        # fetch inputs
        expected_channels = self.opt_in_channels + self.sar_in_channels
        if x.shape[1] != expected_channels:
            raise ValueError(
                f'Input channel mismatch: got {x.shape[1]}, expected {expected_channels} '
                f'(opt={self.opt_in_channels}, sar={self.sar_in_channels})')
        x_img, x_aux = torch.split(x, (self.opt_in_channels, self.sar_in_channels), dim=1)
