...
from models.modeling.backbones.utils import (
    PatchEmbed,
    window_partition,
    window_unpartition,
)

from models.modeling.sam2_utils import DropPath, MLP
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        if self.window_size == 0:
            ...
            """
            1. 实现原理
            动态激活图生成网络：self.dynamic_map_gen 是一个由多层卷积组成的小型神经网络。这个网络能够根据输入的特征图动态生成激活图，而不是固定的卷积核输出。这使得激活图能够更加灵活地适应不同的输入。

            自适应性：由于激活图是根据当前输入的特征图生成的，因此它能够更好地捕捉不同输入之间的差异，从而提升模型对复杂变化的捕捉能力。

            2. 优势
            自适应生成：动态激活图生成网络使得激活图不再是固定的，可以根据每个输入图像的特征动态调整，这提升了模型的灵活性和泛化能力。
            更强的特征表示：通过引入更多的非线性层，动态激活图生成网络能够捕捉到更复杂的时空关系，增强模型在处理复杂变化时的表现。
            """
            # 动态激活图生成网络
            self.dynamic_map_gen = nn.Sequential(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.window_size == 0:
            ...
            # 动态激活图生成
            x0_1 = torch.cat([x0, x1], dim=1)
            activate_map = self.dynamic_map_gen(x0_1)
            activate_map = torch.sigmoid(activate_map)
