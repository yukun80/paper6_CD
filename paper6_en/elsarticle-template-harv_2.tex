...
\begin{figure}[H]
  ...
  \caption{Complete Harmonized Change Encoder. Shared CNN--FPN and frozen DINO features form the bi-temporal pyramid; paired calibration, local alignment, and change query interaction produce multiscale change features.}
...
\begin{figure}[H]
  ...
  \caption{Omni-Scale State-Space Change Decoder. Deep change features are fused and processed with multiscale state-space context before progressive recovery with shallow detail.}
  \label{fig:oscd}
\end{figure}
...
The scene comparison uses bi-temporal SAR imagery and flood-change reference labels for Zhengzhou, Zhuozhou, and Guangxi. Zhengzhou and Zhuozhou use GF3 imagery, while Guangxi uses LT-1B imagery. Observation parameters are listed in Table~\ref{tab:gf3_eval}, and terrain data are described in Section~\ref{sec:depth_data}. Zhengzhou and Zhuozhou use image pairs from different years, whereas the Guangxi pair was acquired in May and July 2026. The reference labels support spatial comparison with predictions, and local inspection regions are shown in the qualitative comparison figures.
...
Figure~\ref{fig:qual_zhengzhou} shows the pre-event and post-event SAR images, reference labels, and flood-change predictions from HarmoSSM and the eight comparison methods in Zhengzhou. The lower local enlargements allow the inundated-area distribution and boundary positions to be compared across methods.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth,height=.40\textheight,keepaspectratio]{figure/Figure6.jpg}
  \caption{Flood-change detection results and local enlargements in Zhengzhou.}
  \label{fig:qual_zhengzhou}
\end{figure*}

Figure~\ref{fig:qual_zhuozhou} shows the bi-temporal SAR images, reference labels, and predictions from the compared methods in Zhuozhou. The full-scene results and local enlargements together show differences in regional coverage and local spatial details.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth,height=.40\textheight,keepaspectratio]{figure/Figure7.jpg}
  \caption{Flood-change detection results and local enlargements in Zhuozhou.}
  \label{fig:qual_zhuozhou}
\end{figure*}

Figure~\ref{fig:qual_guangxi} shows the bi-temporal SAR images, reference labels, and flood-change predictions from the compared methods in Guangxi. The full-scene comparison shows the distribution of predicted inundation and its correspondence with the reference labels.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth,height=.40\textheight,keepaspectratio]{figure/Figure8.jpg}
  \caption{Flood-change detection results in Guangxi.}
  \label{fig:qual_guangxi}
\end{figure*}

\subsection{Conditional depth-estimation results}

Depth results are evaluated through spatial distribution, valid output coverage, and error within a reference domain. Figure~\ref{fig:depth_overview} summarizes CFDepth and FwDET products for Zhengzhou, Zhuozhou, and Guangxi, including valid-depth ratios, WSE-gradient distributions, and a cross-source comparison; Figure~\ref{fig:depth_brazos} further presents spatial errors, estimate--reference relationships, and statistics in the Brazos reference domain. These evaluations are conditional on the supplied support, valid DEM, and reference data and do not provide joint accuracy validation of the complete HarmoSSM--CFDepth chain.

The valid-depth ratios in Figure~\ref{fig:depth_overview} are 37.83\% for FwDET and 93.89\% for CFDepth in Zhengzhou, 61.26\% and 91.70\% in Zhuozhou, and 40.79\% and 94.11\% in Guangxi, respectively. These ratios describe the spatial extent with positive depth, while the WSE-gradient and cross-source panels provide complementary views of surface continuity and product distributions; they are not used to infer independent contributions from the detection modules.

\begin{figure*}[!t]
  \setcounter{figure}{9}
  \centering
  \includegraphics[width=\textwidth,height=.66\textheight,keepaspectratio]{figure/Figure10.jpg}
  \caption{Flood-depth estimates and summary statistics for the three study areas: (a) CFDepth and FwDET spatial distributions; (b) valid-depth ratios; (c) WSE-gradient distributions; (d) cross-source flood-depth comparison.}
  \label{fig:depth_overview}
\end{figure*}

The common-valid Brazos comparison is shown in Figure~\ref{fig:depth_brazos} and Table~\ref{tab:cfdepth_validation}. Positive-depth coverage is 94.570\% for the CFDepth-labeled product and 46.650\% for FwDET; on common-valid pixels, MAE is 0.828 and 1.099 m and RMSE is 1.100 and 1.359 m, respectively. Signed Bias, defined as prediction minus reference, is -0.139 and -0.895 m, while squared Pearson correlation is 0.072 and 0.052 and NSE is -1.014 and -2.032. These values compare the supplied products under the stated support and reference conditions; the bias bars in the summary panel show magnitudes, whereas the table retains the signed definition.

\input{tables/table_cfdepth_validation.tex}

Lower aggregate error does not imply complete representation of reference depth variation. The scatter, error-map, and WSE-gradient panels in Figure~\ref{fig:depth_brazos} provide spatial and distributional context, while negative NSE indicates that squared error exceeds that of a constant prediction equal to the common-domain reference mean. The evaluated rasters remain tied to the supplied reference and support conditions, so their differences cannot be attributed to a single CFDepth operation or generalized to measured urban flood-depth accuracy.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth,height=.50\textheight,keepaspectratio]{figure/Figure11.jpg}
  \caption{Spatial errors and quantitative comparison of flood-depth estimates in the Brazos region: (a) spatial depth maps; (b) estimate--reference density scatter; (c) depth-error maps; (d) WSE-gradient cumulative distributions; (e) coverage and accuracy metrics.}
