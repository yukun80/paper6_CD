...
> 更新日期：2026-09-08。本轮重构第四章并统一英文共享表格；按作者指定的20260902结果恢复9方法五指标，来源核验限制保留。最新图表格式记录见7.35，第四章重构见7.34；第三章与工作图状态见7.33。
> 本轮修改两稿、两份现用表格的模型显示名、图2—5提示词和方案记录；JPG、算法及实验产物不变，在TeX同目录串行编译。
> 图3—5现为已重绘的工作图，局部连接待修；图2和图11继续临时显示旧图。图3—5图注以模块组成和关系为主并简述待修状态，同名TXT提供修正提示词，未核验统计不作为已验证结论。不运行训练、推理、新增评价或栅格重算，不修改算法、数据、权重和实验产物，不提交或推送。

核心问题：**复杂背景下，大范围淹没与弱小、狭窄、破碎洪水并存时，如何利用双时相 SAR 识别新增淹没，并利用所得范围与 DEM 估算水深。**

洪水制图由新增受灾范围识别和范围内水深估算两个相衔接的阶段组成；HarmoSSM承担主要检测方法贡献，CFDepth承担给定范围下的水深估算阶段。跨传感器属于实验条件，不预设为标题或全文创新主轴。“弱小”在本方案中分别指可能的低对比观测和小空间对象，二者不等同；“狭窄”“破碎”“大范围”用于组织已有案例，不暗含已经完成对应类别的全景定量评价。
...
| ③ 方法压缩 | 原稿第9–20页包含较多成熟技术说明及旧预测头 | 当前方法七个二级节；旧方案 §5、§10 出现八节／九节互相冲突 | 保留当前有效实现；按可比观测、变化证据、空间重建三个科学功能组织检测；CFDepth 单列第四节 | Overview独立为3.1，原表征与CQI合并，随后范围重建与水深估算；必要输入、公式、损失和复现信息有明确去处 |
...
- 将覆盖和平滑视为水深准确性，或将 Brazos 条件水深评价当成 HarmoSSM 端到端评价。
...
| 3 Methodology | 3.1 Overview＋三个技术小节，无章首导读 | 当前输入、编码、HA、CQI、OSCD、训练目标及CFDepth；成熟技术只保留必要说明 | 明确范围与水深输出不同，分别进入评价 |
| 4 Experiments and Results | 4.1 Experimental configuration and evaluation protocol（含数据、实现优化、指标协议三个三级节）；4.2 Quantitative flood-change mapping results；4.3 Qualitative comparison in complex scenes；4.4 Conditional depth-estimation results | 合并实现／指标；总体比较后接两景案例，最后条件水深；不保留无实测支撑的独立消融结果节 | 从观察转入解释、应用含义和局限 |
...
### 2.2 Overview与三个技术小节及内容归属

| 编号 | 中文／英文标题 | 合并内容 | 必须保留的复现要点 |
|---|---|---|---|
| 3.1 | 总体框架／Overview | 三段：完整任务、HCE与OSCD检测、CFDepth估深 | 输入与二值目标、范围输出定义；无章首导读 |
| 3.2 | 协调变化编码器（HCE）／Harmonized Change Encoder (HCE) | 六主题：整体任务、共享表征、统计校准、局部对应、状态与差异、查询交互输出 | 保留HA/CQI公式；明确仿射后恢复、尺度路径、Z_i与T_i、双向信息流和token LN复用 |
| 3.3 | 全尺度状态空间变化解码器（OSCD）／Omni-Scale State-Space Change Decoder (OSCD) | 六主题：重建需求、深层融合、多分辨率输入、上下文汇聚、浅层恢复、训练目标 | 明确F→G→O→S→J及六路R3重建；扫描输入公式迁入3.3；SegMAN出处、五级直接辅助监督及4.1配置保留 |
| 3.4 | 范围与地形约束的一阶水深估算（CFDepth）／First-Order Depth Estimation Constrained by Inundation Extent and Terrain (CFDepth) | 五主题：有效支撑与边界、候选筛选、锚点构建、传播初始化、连续性与水深 | 两阶段局部中位数筛选、内边界统计、可达路径与最大距离、回退标记、固定锚点／地形下界及NoData |


Overview独立为3.1，以三段连接 `双时相SAR → HarmoSSM → 新增淹没范围 → 范围＋DEM → CFDepth`。标准骨干、FPN、SS2D与通用损失引用原文并简述采用方式；关键设计和必要公式留正文，细配置集中在紧凑实现表，训练超参数归4.1。

语义 section label 尽量保留。合并后的 `sec:encoder`、`sec:ha` 等旧定位若仍被引用，应指向相应新节或删除冗余引用并同步两稿；不能留下失效标签，也不为匹配数字机械重命名。

### 2.3 当前图件、TXT与引用映射

下表替换此前GF3图在第二章的编号安排。JPG和同名TXT采用唯一临时名两阶段迁移；图片字节内容不变，稳定label保留。图2为旧版临时示意图，图3—5为局部连接待修的重绘工作图；当前状态与修正清单见7.33。
...
引言I6以完整段落概述研究目标、HarmoSSM整体思路及CFDepth第二阶段估深关系；I7用三条贡献依次说明HCE协调变化编码、OSCD范围重建、CFDepth条件水深估算。前两条属于HarmoSSM主要方法，第三条为水深估算阶段。C1—C3继续用于内部证据追踪，不把模块组合或流程连接本身列为独创。

| 层次 | 可以陈述的贡献 | 依据 | 必须避免的扩张 |
|---|---|---|---|
| C1 主要方法：HCE协调变化编码 | HarmoSSM将成对校准和局部对应处理置于变化交互之前，用灾前后状态及差异共同组织新增淹没证据 | 当前HA/CQI代码；与L7、L8具体路径的关系 | 宣称消除了SAR散射差异、恢复了阴影／叠掩区域，或仅凭完整模型分数证明HA作用 |
| C2 主要方法：OSCD范围重建 | 以D3—D5建立区域上下文，再结合D2/D1逐级恢复位置线索，重建像素级新增洪水范围；辅助监督独立接HCE变化特征 | OSCD与训练目标实现；已有图7／8的可见空间表现 | 将采用SS2D、FPN或基础模型本身当创新；声称已量化提高弱对比、宽度、破碎度专项性能 |
...
- **英文：HarmoSSM for Bi-Temporal SAR Flood Change Detection with Change-Defined Depth Estimation**
- **中文：HarmoSSM 双时相 SAR 洪水变化检测及新增淹没范围内的水深估算**

两稿的`\title`及`pdftitle`同步。标题前置HarmoSSM及变化检测，水深作为水深估算阶段；Change-defined指由变化检测确定的新增淹没范围，不表示水深随时间的变化，也不表示HarmoSSM直接回归水深。标题省略DEM约束，正文仍保留CFDepth的地形输入、给定范围和一阶估深假设。依据为用户确认文本、`0626SCI题目润提示词.docx`的贡献主次与准确表达约束，以及`CFDepth/CFDepth_0826.txt`开头对输入和floodSupport的定义。
...
| A4 主要方法 | HarmoSSM先协调成对表征，再联合状态与差异，经区域上下文和局部细节重建范围 | I6；现有实现 | W-C；动作顺序真实，不列骨干、层号或查询数；无组件效果归因 |
| A5 水深估算阶段 | 给定范围、有效边界与DEM支持一阶条件估深 | I5、I6；CFDepth v2 | W-C；HarmoSSM承担主要检测贡献，CFDepth承担第二阶段估深；保持地形与支持区条件 |
...
| I2 新增淹没判别 | L4、L7、L8、L26；HA／CQI代码 | 灾前参考→深度学习时相建模→已有研究在后处理中额外筛除永久水体→状态与差异协调需求 | 段末提出可比较的成对表征及信息协调，引出I3尺度形态；对应I7① | W-I、W-L、W-G；不用例如，不提前介绍本文模块 | Misra后处理支持额外筛除事实，不证明外观差异为其唯一原因，也不声称本文已消除永久水体误检 |
...
| I6 研究工作概述 | 当前HarmoSSM与CFDepth方法正文、指定AWCA-Net引言结构 | 完整交代两阶段制图目标→第一阶段检测→范围输入→第二阶段条件水深；不列性能数字 | 接I5估深需求，引出I7贡献 | W-C：总体概括，区分流程与创新 | 只说明真实设计和用途，不引入独立模块效果 |
...
本四段规划替换此前七段。训练资料、标签、划分、预处理和Brazos评价统一位于4.1；第二章只保留场景及观测、地形资料。`sec:depth_data`仍为2.2，方法末尾涉及评价产物来源的引用改指4.1。资料表1、训练划分表2、实现配置表3、定量比较表4、水深比较表5；新增划分表后的当前编号见7.34。

### 4.4 Methodology：Overview及三个技术小节逐段安排

本表替换原M11—M44拆分方式。公式后的符号解释及作用说明属于相应主题，不新增编号子节。各段采用处理需求、具体关系或输出意义组织叙述；设计目的为D，计算关系为F，不能据此推定专项性能。

| 段落 | 证据位置 | 中心信息与衔接 | 审查边界 |
|---|---|---|---|
| O1—O3 总体框架 | 当前两稿3.1、HarmoSSM及CFDepth接口 | 总体任务→HCE/OSCD检测→范围内条件估深 | 三段原文保留，无章首导读 |
| H1 任务与整体设计 | ha_cqi组装与五尺度接口 | 双时相状态及变化→共享表征、成对协调、查询交互→五级变化输出 | HCE输出为变化特征 |
| H2 空间与语义 | semantic_encoder、dino_adapter；图3 | CNN--FPN浅深层关系，冻结DINO融合P3—P5，时相内融合 | 同类跨时相共享，不混合两时相；光学预训练语义不是SAR散射机制 |
| H3 统计校准 | harmonized_alignment | 当前影像对共同均值／方差→共享仿射→仿射后残差恢复 | 不误写为训练集累计统计 |
| H4 局部对应 | deformable_alignment、HA尺度路由；图3 | 灾后查询、灾前采样键值，偏移与得分偏置，灾前残差更新 | 灾后在局部对齐阶段不变；P1/P2先校准，P3仅对齐，P4/P5旁路 |
| H5 状态与差异 | CQI pair_projection；图4 | 四类输入投影为密集Z_i，再展平T_i | 特征差不直接等于后向散射差或水深 |
| H6 查询与输出 | CQI两次MHA、FFN及out_projection | 查询汇聚→信息返回token→残差FFN→密集残差→D1—D5 | 查询参数独立、query/token LN分开、token LN复用，无跨尺度查询通信 |
| D1 范围重建需求 | OSCD整体流程；图5左侧 | 深层区域信息→逐级局部定位→范围输出 | 需求与结构解释，不写已验证的边界收益 |
| D2 深层融合 | OSCD context_projections/context_fusion | D3—D5各自投影调整成E3—E5→拼接融合F | 保留SegMAN具体结构出处 |
| D3 多分辨率输入 | region_stride2/4、pixel_unshuffle及reductions；图5右下 | 三路共同网格→通道压缩→G | eq:oscd_scan_input迁入且只出现一次，具体配置仍在4.1 |
| D4 上下文汇聚 | state_space_scan与_build_context；图5右上 | G→O→上采样S→投影J；S、Short(F)、Global3(F)、三个E_i+J重建R3 | 严格六路，J不直接拼接，全局支路恢复尺寸 |
| D5 浅层恢复 | detail_projections、reconstruction与head | R3结合D2生成R2，再结合D1生成R1与logits | 不声明显式拓扑或宽度保持 |
| D6 训练目标 | engine及当前损失定义 | 主监督、五级直接辅助监督、支撑保持和粗尺度一致性 | 总目标在方法，定义权重及调度在4.1 |
| W1 支撑与边界 | cfdepth.py support/dilation/erosion | 范围与有效DEM交集，内外边界 | 范围决定位置，地形提供高程约束 |
| W2 边界筛选 | cfdepth.py boundary_candidate与局部中位数 | DEM／坡度筛选→局部中位数→逐点偏差→偏差场局部中位数→容差 | 不是标准单窗MAD，不识别永久水体接口 |
| W3 锚点构建 | dry/wet median与paired_boundary | 内外边界邻域中位数配对平均，施加地形下界 | 湿侧仅内边界样本，锚点由地形推得 |
| W4 初始水面 | _propagate_nearest_anchor与fallback | 支撑内八邻域最近可达锚点，最大距离条件，低置信回退 | 传播结果仅作初始化，栅格可达不等于水力连接 |
| W5 连续性与水深 | _continuity_refinement与depth | 支撑内邻域更新、固定锚点、地形下界、最终水面减DEM | 保留初始低置信标记，支撑外NoData；不据当前方法为既有产物归因 |

### 4.5 Experiments and Results：四个二级节与英文共享表格

以7.34为当前执行记录。SCGC-Net实验章用于借鉴数据／指标／设置与定量／定性比较的组织；`doc/3结果与讨论写作提示词_最后.docx`用于目的—论点—证据—解释—小结的段落逻辑。不把参考文的消融、域适应、复杂度实验或组件因果结论移植到本文。

| 小节／主题 | 资料依据 | 正文安排 | 结论边界 |
|---|---|---|---|
| 4.1.1 数据集与数据准备 | 20260826运行data_snapshot/options、第二章资料、现有GF3及Brazos产物 | 数据用途→来源及4737/1593划分表→256×256样本与三通道→GF3评价场景及图6→独立水深资料 | 不写纯Sentinel-1训练或已证明的郑州独立迁移，B3/B6保留 |
| 4.1.2 实现与优化设置 | 现有代码、options、selection、第三章定义 | 英文分组配置表→输入转换→OSCD具体配置→损失及调度→运行选择与八种比较方法→CFDepth参数 | 256样本与512语义输入区分，单一运行设置不代表所有baseline共同协议，不虚构硬件或软件版本 |
| 4.1.3 评价指标与协议 | 已有指标定义与水深评价脚本 | P/R/F1/IoU/OA及正类有效域→来源口径→正深度覆盖域、共同有效域误差→Pearson平方相关与NSE | 数值转录核对不等于原始评价链核验，不用NoData填0，不混用r²与NSE |
| 4.2 定量比较 | 作者指定并确认为实测的20260902 TSV | 四段：比较目的与表；郑州重叠和召回／误报取舍；涿州高Precision下覆盖差异；两景共性与限制 | 只用该表9方法的90值；正文说明作者提供及原链未独立核验，历史来源冲突留B1/B2，不据表证明单组件或独立跨传感器结论 |
| 4.3 定性比较 | 图7/8现有参考和误差面板 | 目的与颜色定义→郑州小片窄条→涿州连续区域及局部分隔 | 不推测列与方法对应，不以局部图示作全景形态统计 |
| 4.4 条件水深 | 图9—12、既有Brazos表及CSV | 两类资料的评价目的→GF3空间可用性→Brazos覆盖与共同域误差→低相关、负NSE及版本限制 | 检测和估深证据分开；不恢复未实测扰动、平滑或照片精度 |

表格为两稿共享的英文版本，表题／表头／表注及“Table”名称均一致；方法顺序为FC-Siam-Diff、HANet、SNUNet、BIT、ChangeFormer、CGNet、TTP、ChangeDINO、HarmoSSM。主表左右分郑州／涿州，各列P、R、F1、IoU、OA，两位小数；最好粗体、次好下划线只在这9方法内计算。水深表保留原值与原精度，历史未引用表不复用为新证据。

当前无可核验模块消融，既有模拟扰动、GF3深度Panel A及平滑统计不恢复，不新增实验。第五章及结论原文在本轮保持。
...
| C01 回答问题 | I6、M0--M44 | HarmoSSM新增淹没识别与CFDepth条件水深的关系 | 接C02发现与边界 | W-C、W-G | F／D |
...
| L5／B | Mason, D. C., Giustarini, L., Garcia-Pintado, J., Cloke, H. L. (2014). **Detection of flooded urban areas in high resolution Synthetic Aperture Radar images using double scattering**. IJAEOG 28,150–159. [10.1016/j.jag.2013.12.002](https://doi.org/10.1016/j.jag.2013.12.002) | 淹水建筑附近可能出现增强散射；观测几何、阴影和叠掩影响城市可见性 | 结合高分辨率LiDAR与散射模型；不能写城市洪水必然变亮，不能说HarmoSSM显式反演双程散射 | 历史散射背景依据，不用于本轮引言 |
...
| CFDepth当前设计 | `CFDepth_inference/cfdepth.py:483–673`；`raster_io.py` | 范围与有效DEM→边界高程筛选→锚点→初始化→连续性细化；无永久水体输入 |
| CFDepth实现细节 | `cfdepth.py:313–480`、CFDepthConfig | 八连通分量可达传播、无锚点回退、固定锚点与下界细化；数值及网格处理归4.1 |
...
### 6.2 当前CFDepth方法依据与历史版本边界

当前正文依据CFDepth_inference/cfdepth.py与raster_io.py：输入新增洪水范围和DEM，有效支撑区为二者有效区域交集；坡度及局部高程筛选外边界、内外侧锚点、域内最近可达传播初始化、固定锚点及地形下界的连续细化。元数据与参数归4.1，正文不按平台区分。

旧CFDepth/CFDepth_0826.txt含永久水体辅助筛选，CFDepth/CFDepth_GEE.txt采用更早base-depth补全路线，均仅保留为历史参考；不再控制正文或模型图输入。当前算法不识别永久水体接口，地形筛选不被解释为水体语义分类。AGENTS所列HarmoSSM-CFDepth路径当前不存在，不据此虚构实现。

B4仍追查既有GF3和Brazos产物的实际生成记录，不能因正文改为当前设计就将历史产物视为该设计的运行结果。0.01 m为数值下界，无可达锚点保留低置信标记；栅格连通不等于水力连接。
...
| 支持区边界不完整且高程异常 | 有效DEM、坡度和局部高程偏差筛选；内外侧高程锚点 | 下游设计；与FwDET／FLEXTH存在已有思想关系 | 实现和既有产物可说明流程及条件 | 不称首创；不把单项筛选归因为误差降低 |
...
|---|---|---|---|---|
| 新图2，总体流程 | 双时相SAR→可比表征→变化证据→范围重建→新增淹没mask；mask＋DEM→条件水深 | 约70%空间用于HarmoSSM，30%用于下游；CFDepth内用边界→锚点→初始化→细化小插图；层号移详细图 | 旧Urban限定、Mask2Former、HA内语义融合及旧深度流程 | `figure2.txt`写明科学主线、两个输入阶段及输出含义；Brazos不画成训练输入；不把深度接成网络端到端训练head |
| 新图3，可比表征 | 同一场景灾前后观测→共享CNN／冻结DINO→语义融合→成对校准→局部对应特征 | 清楚画P3–P5融合、P1／P2校准、按配置P1–P3软对齐；两个归一化路径分开 | 旧mu_src／sigma_src目标、语义融合排在HA后；对齐方向不明 | `figure3.txt`统一canonical可学习参数，灾后query／灾前key-value；不是SAR物理辐射校正；压缩编码入口，特征薄片与短算子展开HA校准、采样及残差；无三分区框 |
| 新图4，变化交互 | 一个尺度的pre／post／有向差／绝对差→pair tokens↔learned queries→dense change | 详细画单尺度，旁注五尺度分别处理；中央空间特征主线、上方查询支线和底部残差；数量与通道配置留在正文 | 旧投影写MLP、token-to-query符号与箭头角色不一致 | `figure4.txt`逐项核对投影和两次读写方向；不画16种类别、跨尺度query通信、object queries或Hungarian matching |
| 新图5，范围重建 | D3–D5提供区域上下文，D2／D1逐级恢复细节，形成洪水／背景密集输出 | 左侧整体重建、右上区域上下文、右下多分辨率输入；仅画推理，不画辅助头或训练区 | 当前重绘图左侧尺度关系已正确，右上J再注入、E_i配对、S分流及六路拼接仍待修正 | `figure5.txt`锁定箭头起止、尺度及监督用途；注明MMSCoPE-inspired采用关系；无Mask2Former、set prediction或未实现路由 |

模型图当前使用浅底、细黑箭头、Times类字体、数学斜体及轻微轴测特征薄片，图5保持左主右详的蓝—珊瑚配色；替换早期纯平面块和Arial要求。字号以最终版面可读为准，不指定未经核验的RSE硬性尺寸。
...
| 12 | Brazos既有比较图及其CSV／脚本 | `figure12.txt`说明共同有效域、参考资料、覆盖分母、Pearson r²与NSE；检查图与表同一产物；不当HarmoSSM端到端结果 |
...
- 图2：HarmoSSM占主要空间，范围与DEM进入CFDepth；水深无反向训练连接。
- 图3：编码入口压缩，HA以成对特征薄片、联合统计、仿射、恢复残差、采样与注意力算子展开；P1/P2校准，P3绕过校准，P4/P5直达输出；灾后Q／灾前K/V。
- 图4：空间特征薄片与查询条带区分，MHA₁/MHA₂端口及四条残差显式绘制，LNq与共享LNt区分；单尺度展开，五尺度独立，无三联面板。
- 图5：D3–D5上下文、D2/D1细节恢复及主输出保持；辅助监督在正文解释，图中仅画推理。

按[OpenAI官方提示词指南](https://developers.openai.com/cookbook/examples/multimodal/image-gen-models-prompting-guide)明确用途、画布、文字、布局及约束；这不是对生成准确率或网页输出格式的保证。白底、柔和配色、统一线宽、正交箭头和留白构成基础规范。图3/4采用Times类字体、数学斜体、浅轴测特征薄片和窄算子板，替换上一轮全圆角文字框规范；其他图提示词本轮不变。图3—5已重绘为工作图，局部连接待修正；图2继续显示旧版示意JPG。当前TXT提供完整提示词及针对工作图的定点修订，最新薄片/算子布局替换早期文字框及三面板方案，图5采用左主右详且仅画推理，当前状态见7.33。
...
|---|---|---|---|
| B1 主检测指标 | [TBD: 20260902 TSV对应原始报告、预测／标签版本、阈值、有效域及混淆计数；数值与历史非实测投影记录一致的冲突] | 按用户最新确认恢复作者提供的9方法90个指标及表内数值比较；正文保留原评价链未独立核验。未混入20260901；摘要、第五章和结论不修改 | 作者确认不等于独立复现或来源冲突关闭，不据此声明已核验的公平比较、统计显著性或独立迁移；详见7.34 |
| B2 方法与图表身份 | [TBD: 各baseline及HA-CQI行的checkpoint/预测映射，尤其ChangeDINO与旧IFN身份；图7/8列(d)--(l)版本] | 主表维持用户确认的9方法名称和20260902行，HA-CQI显示为HarmoSSM；标记仅表示表内数值排序。图7/8保留存档列号与误差颜色 | 不将表格行序用作面板列映射；历史IFN与当前ChangeDINO的身份冲突尚未解决，不称已完成公平对照 |
...
| B4 水深产物版本 | [TBD: GF3/Brazos栅格的生成代码版本、参数、支持区、DEM、永久水体及参考垂直基准] | 方法按当前去除永久水体约束的cfdepth.py描述；Brazos表仅转录CSV且说明是既有产物比较；删除评价脚本自动剔除永久水体的错误说法 | 不将产物差异归因当前代码或单个水深操作；不称城市实测深度验证 |
...
| 阶段 | 实际交付 | 六项检查结论 | 剩余阻断 |
|---|---|---|---|
| G1 结构 | 两稿六章顺序、数据第二章、Overview加四个技术小节；12图无覆盖迁移、label保留，图2--5及图11以旧图临时显示；4共享表 | 研究对象和上下游清楚；同序结构与引用可检查；Overview独立为3.1；无结构性冗余 | 新版模型图尚未生成属于B8，旧图显示不等于机制或统计验收 |
...
### 7.26 第二章措辞与表1英文统一（2026-09-07）

本轮仅表1统一英文，撤销此前讨论中的“全文表格统一”选项。第二章保留四段结构，图件与其他表格不变。正文保留“研究区1为河南省郑州市区，以下简称郑州研究区”，英文采用covers the urban area及Zhengzhou study area。

| 修改项 | 涉及文件 | 修改与依据 | 审查结果 | 剩余阻断 |
|---|---|---|---|---|
| 同期影像选择 | 两稿2.2 | 说明使用灾前一年同期（7月）的参考影像；减小季节及农业活动干扰为选取目的 | 不宣称灌溉影响已消除，涿州7月20日与7月31日不写成同日 | 无专项实验，不能推断干扰降低的实测幅度 |
| 模式与极化 | 两稿2.2 | 直接说明FSII及HV并引用表1；正文删除标称10 m采样间距 | 10 m仅在表内保留，Pixel spacing不改称实际分辨率 | B6原始采集参数核验仍未关闭 |
| 英文表1 | table_gf3_eval_dataset.tex | 表题、表头、内容及Table前缀统一英文，删除整块表下备注 | 数值、label和FABDEM分区保持；其他表格不改 | 原备注任务保留于下 |

[TBD: 表1的GF3日期、极化、成像模式及像元间距仍需对应原始采集元数据；删除表下备注不表示已核验。] 本轮不新增实验。当前D3规划以本节措辞为准，7.25中的表下参数备注要求为历史。

本轮检查完成：两稿在paper6_en/内依次latexmk -xelatex编译通过；最终无未定义引用、缺字、Overfull或Underfull警告。已查看中文版第6页及英文版第8页的表1，表题、Table前缀、表头与内容均为英文，无表下备注，列宽正常。中文版PDF无TBD，tab:gf3_eval仍为表1。与任务起始快照逐字节核对，两稿仅指定措辞变化，其他表格与全部图件哈希未变，表1原有参数数值保持。B6核验事项仍保留，不将同期选取目的作为已验证的灌溉抑制结论。

### 7.27 第三章处理逻辑重构与当前CFDepth统一叙述（2026-09-07）

本轮实施“范围识别—范围内水深估算”两阶段叙述。修改依据为已审计HA-CQI代码和当前CFDepth_inference，不修改算法，不新增实验。此前按GEE介绍、永久水体筛选与图中辅助水体输入的要求失效，旧记录仅用于追踪历史。

| 修改项 | 涉及文件 | 修改与依据 | 审查结果 | 剩余阻断 |
|---|---|---|---|---|
| 方法处理链 | 两稿第三章 | 保留四节及正确HA/CQI公式，去掉编码特征差异的物理散射方向解释，突出OSCD区域与细节关系 | 原稿源域原型、Mask2Former和旧补全流程未恢复 | 不据设计推断模块专项效果 |
| 复现迁移 | 两稿4.1、配置表引用 | 归一化、扫描输入、Focal/Tversky与组合、权重调度放在预处理之后；配置表2移入4.1 | 训练数值及完整目标保留，总目标仍在方法 | 已识别运行与比较产物的对应仍属B1—B3 |
| CFDepth | 两稿3.4和4.1 | 范围与有效DEM交集、地形边界筛选、锚点、初始化和细化；移除永久水体输入，域外及无DEM为NoData | 正文不区分平台，方法与当前计算一致 | B4产物来源不因改写关闭 |
| 提示词 | figure2—5.txt | 删除图2额外水体输入及箭头，图3/4配置文字后移，图5扫描细节置紧凑子区 | 未生成或改动JPG；旧示意图状态保留 | 最终图件仍待绘制 |

当前方法保留输入与输出、HA两类处理、CQI交互及残差、区域细节重建、完整总目标和四组水深关系。迁移公式label保持，配置表2在首次引用后位于4.1。4.1顺序为数据预处理→网络与输入配置→扫描实现→损失及调度→优化和选择→CFDepth参数→既有深度评价资料→评价指标。引言中永久水体问题的文献评述及讨论中的真实局限保留，不写CFDepth能自动识别该接口。

[TBD: 既有GF3/Brazos深度产物与当前无永久水体输入设计的生成记录未闭合；评价继续限定为既有产物比较。] B1—B8保留。本轮编译和范围检查结果随后记入。

本轮最终验收：两稿在paper6_en/内依次latexmk -xelatex构建通过，最后日志无未定义引用、缺字、Overfull及Underfull警告；全部原label保留，双语公式、图表和章节label编号对应一致。方法章仍为四节，归一化、扫描输入、Focal、Tversky、分割组合公式及配置表2已迁入4.1预处理之后，总目标保留于方法章。查看中文CFDepth与配置页、英文第15/19页，图表公式完整，无裁切。两稿方法章与4.1之外的正文对照任务起始快照未变；图2—5TXT按当前机制更新，全部JPG、表格源文件及reference.bib哈希保持。中文版不新增TBD，现有文献评述与产物评价限制保留。可实施方法重构完成，旧图重绘及B1—B8证据问题继续阻碍投稿定稿。


### 7.28 图3、图4连续构图提示词（2026-09-07）

采用用户确认的“主线清晰、必要细节”方案。旧JPG的三段虚线边界、独立标题及重复输入输出形成拼接感；此前TXT仍明确要求三面板／三子区域，本轮整体替换这些布局指令，不改变算法信息流。

| 修改项 | 涉及文件 | 修改与依据 | 审查结果 | 剩余阻断 |
|---|---|---|---|---|
| 可比表征构图 | figure/figure3.txt | 双时相轨道贯通，融合先于校准／对齐，共享统计居中；保留各尺度旁路和只更新灾前的关系 | 主提示词独立完整，无三阶段框；必要算子压缩为节点内短注 | 新JPG未生成，最终字号和布线待实际图审查 |
| 变化交互构图 | figure/figure4.txt | 中央空间特征主线、上方查询支线、底部密集残差；四路输入与两次Q/K/V角色明确 | 原始token进入第二次交互，残差加法位于输出卷积前；五尺度独立 | 新JPG未生成，可读性需缩至论文宽度实查 |
| 统一规范与修订 | 两份TXT、本方案6.4/6.6 | 颜色绑定数据角色，统一字体线宽；局部修订按节点和连接定位，旧面板指令失效；纠正现行提示词契约的旧图号 | 不改变两稿、JPG、表格、引用或算法；B1—B8保留 | 旧图仍为临时展示，不视为完成科学图件验收 |

本轮只交付提示词和方案记录，不生成图片。文本检查覆盖无冲突面板指令、融合顺序、尺度旁路、两次交互角色与残差连接；实际生成后的连接正确性及论文宽度可读性仍需验收。正文未修改，不运行LaTeX构建。


### 7.29 新增Overview与两阶段方法导读（2026-09-07）

当前结构改为3.1总体框架／Overview及3.2—3.5四个技术小节，替换旧“Overview不独立设节”的要求。历史四节记录仅描述当时版本。

| 修改项 | 涉及文件 | 修改与依据 | 审查结果 | 剩余阻断 |
|---|---|---|---|---|
| Overview三段 | 两稿第三章 | 借鉴指定AWCA-Net论文§3/3.1的需求—处理目的—顺序流程写法；第一段说明任务与输入输出，第二段检测，第三段条件估深；不新增公式 | 整体制图先于模块介绍，不复制参考论文效果结论 | 原B1—B8保留 |
| 定义与衔接 | 两稿新3.2、图2位置 | 输入符号、二值目标、概率到范围的定义归Overview；通道与归一化留4.1，原技术公式及label保留 | 图2在Overview，四个技术小节自动顺延 | 旧图临时显示状态不变 |
| 提示词定位 | figure2—5.txt使用说明 | 仅同步章节位置为3.1总体框架、3.2表征、3.3交互、3.4重建 | 主绘图指令及JPG不改 | 新图实际生成与审核待完成 |

逐段规划：O1为制图任务与两阶段关系，证据为图2及当前输入输出契约；O2为共享编码—协调—交互—重建及二值输出，证据为HA-CQI现有方法；O3为有效范围—边界锚点—水面—水深，证据为当前CFDepth。O1引出O2，O2输出连接O3，O3之后进入3.2的具体处理说明。章首仅一句导读，不重复三段概述。本轮未发现专门的方法写作DOCX，采用已读取的指定参考论文组织方式，不将引言提示词强套为方法结构。

最终验收：两稿在paper6_en/内依次latexmk -xelatex构建通过，日志无Warning、未定义引用、缺字或Overfull/Underfull。两稿全部label对应编号一致，方法均为Overview加四个技术小节；中文PDF无TBD。目视检查英文第9页与中文第6页，标题和段落排版正常。两稿从原共享编码第二段起至文末逐字节对应文本未变，方法前正文未变；全部JPG、参考文献和表格哈希不变。图2—5TXT只更新章节定位。本轮未新增实验，图件重绘及B1—B8继续保留。


### 7.30 图3/4特征与算子驱动重设计（2026-09-07）

参考SCGC-Net方法§III及图3 CAM、图4 CU的正文—图件分工：目的与公式由正文解释，图用特征、算子、分支和残差呈现计算。用户确认图3压缩编码并展开HA。7.28的连续计算与无三联面板要求继续有效，纯平面文字框、Arial、禁止数学符号/特征厚度的视觉限制由本节替代。7.29方法章节位置保留，图3/4分别对应3.2/3.3。

| 修改项 | 涉及文件 | 修改与依据 | 审查结果 | 剩余阻断 |
|---|---|---|---|---|
| HA模块图 | figure3.txt | 共享编码简写，P1/P2校准、P3直入对齐、P4/P5旁路；依据harmonized_alignment.py及deformable_alignment.py展开联合统计、仿射后恢复残差、偏移/偏置及pre更新 | Recover残差起点为仿射后特征；偏移作用于采样、偏置进入Softmax前得分；输出无时相互连 | 生成后的尺度分流和字号需逐项实查 |
| CQI模块图 | figure4.txt | 依据change_query_interaction.py展开MHA与四条残差，特征薄片/查询条带/短算子/图例替代长句框 | 原始T参与第二次MHA，LNt共享，Z在输出卷积前相加，五尺度独立 | 最终生成图仍待科学与版面验收 |
| 视觉规范与范围 | 两份TXT、本方案 | 白底细黑线、Times数学符号、轻微轴测薄片、局部浅底；主提示词及节点定位修订独立完整 | 未生成JPG，正文与算法不修改，B1—B8保留 | 不宣称图件已可用于投稿 |

本轮不运行LaTeX构建。文件哈希核对限定修改为上述三份文件，其他论文文件（含JPG、TeX、表格、BibTeX）保持；提示词连接已对照当前代码检查，生成后的视觉效果尚未验证。

### 7.31 第二段结尾：永久水体筛除与变化判别问题（2026-09-07）

| 修改项 | 涉及文件 | 修改与依据 | 审查结果 | 剩余阻断 |
|---|---|---|---|---|
| I2结尾 | 两稿第二段、本方案I2 | 采用批准三句：已有额外后处理→外观差异及状态判断→成对可比性与信息协调；删例如及重复区别表述 | 第一段及第二段前半部分不变；段末不提前介绍HA-CQI；引用顺序不变 | 不声称已消除误检或不需辅助资料 |
| 文献边界 | L26／misra2025globalFloods | Misra等Methods—Post-processing明确采用ESA地表覆盖去除永久水体，显式掩膜有助减少预测噪声 | 事实只支持该研究的处理方式；后句外观差异为一般问题解释，不归因为该论文验证的唯一原因 | B1—B8保留，不扩大性能结论 |
| 工程与范围 | 两稿构建产物 | paper6_en串行编译、范围及引用审查 | 两稿编译通过；无Warning、未定义引用、缺字或Overfull／Underfull；授权结尾之外正文逐字节不变 | 无新增实验 |

[原始论文](https://www.nature.com/articles/s41467-025-60973-1)，Methods／Post-processing中永久水体筛除的连续两句为来源定位。已有BibTeX条目不变。本轮保留7.24—7.30的第二章、方法与图件修改，不以旧快照覆盖当前工程。

当前英文Introduction为1412词（剔除引用及LaTeX命令，连字符词计一词）。

本轮最终中文PDF 28页、英文34页，中文第2页与英文第3页检查第二段结尾及第三段衔接正常。全文引用顺序及图表编号一致，中文TBD为0。仅两稿指定结尾、本方案及对应构建产物变化，reference.bib、图件及其他源文件哈希不变。英文Introduction为1412词，不为凑旧预算调整其他段落。`git -c core.whitespace=cr-at-eol diff --check`通过。[本轮范围、引用与日志审查](/tmp/p6_p2end_jihwrx04/audit.json)。


### 7.32 HarmoSSM方法写作与图文同步（2026-09-07）

命名与职责决定继续有效；段落与当前图件状态以7.33为准。本节替换前述五个方法小节、HA/CQI分别作为贡献及旧模型名称要求，历史记录仅用于追踪。HarmoSSM（Harmonized Change Encoding and Omni-Scale State-Space Decoding for SAR Flood Change Detection）对应原HA-CQI检测实现；HCE包括共享CNN–DINO表征、HA和CQI，OSCD重建范围，CFDepth估算范围内水深。代码/目录/checkpoint中的HA-CQI身份不改，新名称不解除产物归属阻断。CRR-Flood候选失效。

| 修改项 | 涉及文件 | 修改与依据 | 审查结果 | 剩余阻断 |
|---|---|---|---|---|
| 两阶段总述及命名 | 两稿标题、元数据、摘要、引言概述与贡献及相关称谓 | HarmoSSM为检测模型，HCE/OSCD分别编码与重建，CFDepth为第二阶段；三条贡献不变成四模块清单 | 无新增性能数字；训练记录保留HA-CQI实现标识 | B1—B8仍有效 |
| HCE | 两稿3.2 | 合并原3.2/3.3，四主题串联；保留校准/对齐/交互/输出公式及label，Z与T分开 | 灾后不变，五尺度独立，两类LN分别定义，token LN复用 | 图3/4局部连接未完成修图 |
| OSCD | 两稿3.3 | 需求→整体组织→上下文→逐级恢复→训练；SegMAN出处位于具体上下文结构句 | 不称全部原创/首次，不把短路池化和再注入写成新机制；辅助头接D1—D5 | 无专项解码效果证据，不新增实验 |
| CFDepth | 两稿3.4 | 保留当前有效范围、筛选、锚点、初始化、细化与深度公式 | 范围与DEM输入不变，无额外水体输入 | B4现有产物版本链仍未闭合 |
| 工作图状态与提示词 | 图2—5TXT、两稿图注 | 保留现有JPG；图3/4为一轮重绘工作图，图5已重绘但右上连接待修；当前修正清单见7.33 | 采用最近薄片和紧凑双层风格，不回退旧布局 | 新生成图逐项验收仍待完成 |
| 表格显示名称 | 方法配置表、总体比较表 | 仅HA-CQI显示名改HarmoSSM，数值与占位横线不变 | 未引用历史消融表保持原状 | B1/B2不因改名关闭 |

图3待修：同类跨时相权重共享，灾后主线不变，K_s→评分、V_s→加权值，Q来自U_B，偏置进softmax前加法，U_B直接通向不变输出，各自原特征进入Norm。图4待修：移除重复X_B→C直连，拼接只含四类输入。图5提示词当前计算已核对，保持O→上采样S→投影J、六路上下文拼接及D2/D1细节恢复，仅画推理。

段落契约：O1—O3为总体任务、HCE/OSCD检测、条件估深；H1—H4为共享表征、成对协调、四路输入、查询交互；D1—D4为洪水空间重建需求、区域上下文、浅层恢复及输出、完整训练目标；W1—W3为有效支撑与筛选、锚点初始化、细化及深度。显示公式后的解释属于对应主题，不新增编号子节。来源按现有代码、图5提示词及已读SegMAN原文/作者实现核验；不将图3错误用作正文机制依据。

[TBD: 图3/4连接错误及已重绘图5右上连接未完成修正验收，投稿定稿仍受阻。] 其他B1—B8继续保留。编译与范围核对结果随后记录。

最终验收：两稿已在paper6_en/内依次latexmk -xelatex构建通过，最终日志无Warning、未定义引用、缺字或Overfull/Underfull。全部原label保留，双语label编号一致，sec:encoder/sec:ha/sec:cqi均指3.2；方法章四个二级节。中文PDF无TBD。目视英文标题页及第13页、中文第10页，标题、图4/5和重建公式无裁切。研究区与数据整章、CFDepth技术内容保持不变；全部JPG/PNG及reference.bib哈希一致。非构建变更仅两稿、两张现用表格显示名、图2—5TXT和本方案。图3/4连接修正与已重绘图5右上连接验收尚未完成，B1—B8仍阻碍投稿定稿。


### 7.33 第三章深化：解释与关键公式并重（2026-09-08）

本节替代7.32的四／四／三主题压缩要求，名称与三阶段职责继续沿用。按已批准方案借鉴SCGC-Net方法章的组织：处理需求→整体设计→信息流→关键公式及符号解释→输出与后续衔接。方法章保持3.1总体框架、3.2协调变化编码器（HCE）、3.3全尺度状态空间变化解码器（OSCD）、3.4范围与地形约束的一阶水深估算（CFDepth）四个二级节。3.1三段原文保留，不恢复章首导读，不新增通用Mamba预备知识。

| 修改项 | 逐段组织与依据 | 审查结果及边界 |
|---|---|---|
| HCE六主题 | H1双时相任务与变化输出；H2共享空间／语义表征；H3成对统计与仿射后残差恢复；H4局部对应及尺度路径；H5状态／差异密集构建；H6两次查询交互及输出 | 现有HA/CQI公式和label保留。灾后不变仅指局部对齐阶段，P1/P2仍先共同校准；P3仅对齐、P4/P5旁路。Z_i与T_i、独立查询参数及token LN复用明确 |
| OSCD六主题 | D1重建需求；D2深层投影融合；D3多分辨率扫描输入；D4四方向上下文及六路汇聚；D5逐级细节恢复与预测；D6完整训练目标 | 补深层融合、O→S→J及六路R3公式；迁移eq:oscd_scan_input并以G统一图文符号，4.1保留具体核大小、通道、尺寸、填充和扫描实现。SegMAN引用位于具体上下文组织处，不声明通用支路全部新增 |
| CFDepth五主题 | W1有效支撑与内外边界；W2候选筛选；W3内外边界中位数与锚点；W4最近可达锚点初始化与回退；W5连续性细化与水深 | 新筛选式按cfdepth.py的局部高程中位数→逐点偏差场→偏差场局部中位数→容差筛选表达，未简化为标准MAD。内侧中位数只用内边界；八邻域传播、最大距离、低置信标记、固定锚点及地形下界明确 |
| 图3—5图注／TXT | 图注以模块组成、计算关系、必要符号为主体，末尾简短保留工作图状态；无虚构面板字母 | 图3/4既有修图清单保留。图5更新为已重绘、右上待修，增加对现图删除错误线后重连的英文定点提示词；JPG不变 |
| 受保护范围 | 只改两稿第三章、4.1必要公式迁移及配置衔接、图3—5TXT和本方案 | 标题、摘要、引言、研究区与数据、结果及实验数值保持原文；不新增实验、验证统计或性能结论，B1—B8保留 |

图5当前待修：J分别进入三个加法输入，不直达最终C；E3/E4/E5各自对应其加法，删除E5进入首个加法及F进入加法的错误线；S直达C与三个加法结果分离，删除Up总线与E3+J结果的错误结点；Short Conv显式取F；最终C严格六路，删除重复路线。保留已正确的左侧恢复、右下多分辨率输入及O→Up→S→Proj→J。工作图验收与正文正确性分别记录，正文深化不解除图件或投稿证据阻断。

验证完成：在`paper6_en/`内依次运行`latexmk -xelatex -interaction=nonstopmode -halt-on-error elsarticle-template-harv_2.tex`及`latexmk -xelatex -interaction=nonstopmode -halt-on-error Manuscript_revised_cn.tex`，均通过。最终日志无Warning、未定义引用、缺字、Overfull或Underfull；`git -c core.whitespace=cr-at-eol diff --check`通过。中英文各47个label，原43个全部保留，新添4个公式label，双语编号一致；图3—5编号、公式迁移及四个方法二级节检查通过，中文TBD为0。

实际篇幅：英文全文37页，方法章位于第9—20页；中文全文30页，方法章位于第6—14页。方法正文按剔除公式、行内数学、图注、节标题、引用及LaTeX命令的口径，英文约2372词，中文约3861个汉字。主题组织为O3／H6／D6／W5，显示公式后的解释与其主题合并计算。目视检查英文第11、14、15、17、18页及中文第13页，图3—5图注、上下文公式及边界筛选式无裁切或编号碰撞；工作图自身连线错误仍按上述清单保留待修状态。

范围检查以本轮编辑前快照为基准：3.1及其前全部正文逐字一致，4.1除扫描公式迁移和对应配置衔接外逐字一致，后续结果、讨论和结论逐字一致。88份论文目录文件及4892份`HA-CQI/outputs/`文件建立SHA-256基线；其中全部12张JPG、其他受保护论文图件、表格、BibTeX与全部4892份输出文件哈希不变。实际源文件改动仅两稿、图3—5TXT和本方案，另有相应LaTeX构建产物；英文CRLF与中文LF保持。未运行训练、推理或新增评价。B1—B8继续有效。

[本轮标签、范围、篇幅与哈希审查记录](/tmp/p6_deepen_jncxpszx/audit.json)。正文重构已完成，图3/4连接及图5右上连接尚未修图验收，投稿证据亦未闭合。


### 7.34 第四章重构与英文共享表格（2026-09-08）

用户已确认实施：两稿第四章同步，所有本轮实验表格共用英文；使用`HA-CQI/outputs/gf3_segmentation_metrics_v20260902.tsv`，维持9方法，主表两景左右分组各五指标；4.1细分三个三级节，其余为定量、定性、条件水深三个二级节。SCGC-Net实验章（PDF第7—11页）提供组织与宽表参考，指定结果与讨论DOCX提供段落写作逻辑。未移植参考文的额外实验，也未照抄其印刷有误的IoU分母。

来源记录与作者确认分开保存：规划阶段核对发现，当前20260902 TSV的120个指标单元与历史明确标记`synthetic_subjective_projection`／`measurement_status=not_measured`的投影记录全部相同；历史IFN两行在当前表中名为ChangeDINO。对应投影TSV／meta文件当前未找到，不能写成现存附件，也不能据数值重合推定是谁何时改名。用户随后明确“20260902就是真实实测结果”，并要求忽略20260901。故本轮严格按指定表转录，以作者提供并确认为实测的结果口径呈现，表注声明原始评价链未独立核验；上述冲突和B1/B2未关闭。既有20260901报告虽可对应另一组结果，未进入本轮正文、表格或差值计算。

| 修改项 | 实际实现 | 审查要点 |
|---|---|---|
| 数据与设置归拢 | 4.1.1新增训练来源／划分表；4.1.2扩展HCE/OSCD、优化选择、CFDepth英文配置表；4.1.3独立评价指标及域定义 | 4737/1593来自关联快照；256与512用途不同；无未经记录的硬件或统一baseline设置 |
| 定量主表 | 9方法×2场景×5指标，所有值取20260902；HA-CQI仅显示名为HarmoSSM | 两景各P/R/F1/IoU/OA；最好／次好按9方法独立计算；百分数不再次乘100 |
| 4.2四段论证 | 目的与表→郑州→涿州→两景总结；数值、指标权衡与作用解释连接 | 郑州对CGNet IoU/F1差6.73/4.61百分点；涿州差4.31/2.53；不称所有指标最优、显著性或组件因果 |
| 4.3与4.4 | 先交代图示目的和评价域，再解释空间误差、覆盖／共同域误差／低相关及负NSE | 不推测面板身份，不把检测和独立Brazos水深组合为端到端精度 |
| 英文共享表 | 新建训练划分表，配置表和主表英文重构，水深表去除中文分支 | 全部表题、表头、表注及Table名称为英文；水深数值和精度不变，无新增TBD进入中文稿 |
| 字面范围 | 两稿第四章、四份当前实验表及本方案 | 第二、第三章及第五章起至文末逐字保持；JPG、算法、源TSV及其他实验产物不变；无新增实验 |

当前训练设置核对：20260826运行options记录input_size=256、batch=12、epochs=80、lr=1e-4、weight_decay=5e-4、head_lr_mult=2、bf16、seed=1、shared radiometric mode；metrics.jsonl有epoch1—80，selection选epoch77／threshold0.80，搜索0.05—0.95步长0.01。AdamW和cosine由当前engine实现支持。gpu_ids=[0]不证明设备型号，未补写硬件及环境版本。这些记录仍未替代20260902每行独立评价协议。

验证完成：在`paper6_en/`内依次执行`latexmk -xelatex -interaction=nonstopmode -halt-on-error elsarticle-template-harv_2.tex`和`latexmk -xelatex -interaction=nonstopmode -halt-on-error Manuscript_revised_cn.tex`，两稿通过；最终日志无Warning、未定义引用、缺字、Overfull或Underfull。`git -c core.whitespace=cr-at-eol diff --check`通过。

数值验收：18条场景—方法记录共90个指标值与指定TSV逐字一致，10处最好和10处次好标记按所选9方法验证通过；四个IoU/F1百分点差值正确。水深表两行全部原数值及显示精度保持。当前四份实验表的表题、表头、表注及Table名称均为英文，两稿共用相同文件；主表没有缩放图像或缩小到不可读字号。原47个TeX内标签及所有原公式内容保留，新增3个三级节标签和1个数据表标签；双语完整label编号一致。第四章四个二级节、4.1三个三级节、4.2四段核对通过，中文TBD为0。

实际篇幅：英文全文40页，第四章第21—31页；中文全文31页，第四章第15—24页（第24页随后进入讨论）。第四章正文按排除公式、行内数学、图表、标题及LaTeX命令的口径，英文约2197词，中文约3516汉字。目视英文第23/27页和中文第15/17/20/24页，数据划分表、分组配置表、双景五指标宽表及水深表无裁切、重叠或编号碰撞，英文表格在中文稿显示正常。

范围验收以本轮开始时快照为基准：两稿第四章之前和第五章起至文末逐字一致，所有figure环境逐字一致；12张JPG和4891份`HA-CQI/outputs/`文件SHA-256不变，源TSV、表1、BibTeX及其他受保护论文资料均未改变。实际源文件变更为两稿、配置表、主比较表、水深表、本方案及新建的训练划分表，另有对应LaTeX构建产物。英文CRLF和中文LF保持。未运行训练、推理、新增评价或数据重算。

[数值、标签、范围、哈希与篇幅核对记录](/tmp/p6_exp_bpqf668c/audit.json)。第四章与英文表格重构已完成；B1/B2的来源／身份冲突、B3的空间独立性及其他B项按上述边界保留，不将排版与转录验收视为原始实验链已验证。


### 7.35 实验图表标题与指标单位（2026-09-08）

按用户要求删除四份实验表格底部备注，定量主表第二行十个指标标题逐一补充(%)；保留90个数值及粗体／下划线标记。参照SCGC-Net表I及图5/6的“比较对象／结果＋数据集／场景＋面板或颜色解释”组织，重写表2—5和图6—12标题，删除标题中的Author-supplied、identified run、archived、临时展示和核验状态式措辞。图题保留必要面板、颜色、局部图和WSE符号含义；不补造图7/8的方法列映射。

表格备注删除不改变原始实验来源状态。正文现有作者提供结果口径、评价链限制、面板身份、水深产物版本及WSE统计限制保持原文，7.34的来源冲突与B1—B8继续保留；本节只更新其图表表达要求。第二、第三章图题和表1保持。当前稿四份实验表仅保留英文标题／表头，图6—12中英文图题同步。

两稿已在paper6_en内依次latexmk -xelatex编译通过，最终日志无Warning、未定义引用、缺字或Overfull/Underfull；diff --check通过。英文40页、中文31页；目视英文第27页和中文第20页，新增百分号的十列指标表头均可读，无挤压或裁切。四表下方备注已全部删除，表格原数据行及最优标记保持。两稿除图6—12 caption外逐字一致，所有label和双语编号保持，12张JPG、参考文献及其他受保护论文文件哈希不变。具体记录见[本轮审查](/tmp/p6_captions_5uhgiw7j/audit.json)。
